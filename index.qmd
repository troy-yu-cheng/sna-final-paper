...
Structural metrics are computed for each of the 18 amplification backbone networks. To test the proposed hypotheses, one-way ANOVA is applied to compare differences across the three network types for each structural metric. Where significant differences are detected, post-hoc pairwise t-tests with Bonferroni correction are used to clarify specific group differences.

For all statistical tests, a significance level of p < 0.05 is used. Effect sizes are reported to provide additional interpretation of practical significance alongside statistical significance.

To validate the structural relevance of clustering patterns, Conditional Uniform Graph (CUG) tests are conducted. These tests assess whether observed transitivity values significantly exceed random graph expectations, confirming that clustering patterns arise from meaningful co-dissemination behaviors rather than random chance (Faust, 2006).
